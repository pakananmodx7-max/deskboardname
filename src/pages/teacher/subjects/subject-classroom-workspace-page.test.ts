...
describe('Subject-Classroom workspace tabs — Topics removed, ตรวจสอบงาน+คะแนน merged into ตรวจงานและคะแนน, งาน removed as a top-level real tab', () => {
  ...
  const expectedRealKeys = ['overview', 'students', 'attendance', 'lessons', 'checkAndGrades', 'sgsScores']
  ...
  it('real workspace is exactly ภาพรวม/นักเรียน/เช็กชื่อ/บทเรียน/ตรวจงานและคะแนน/คะแนน SGS, in that order — งาน is no longer a top-level tab (assignments now live as matrix columns inside ตรวจงานและคะแนน), ตรวจสอบงาน/คะแนน are no longer separate top-level tabs either, and คะแนน SGS is its own independent tab (never merged into ตรวจงานและคะแนน)', () => {
    ...
    const sgs = REAL_TABS.find((tab) => tab.key === 'sgsScores')
    expect(sgs?.label).toBe('คะแนน SGS')
  })

  it('ตรวจงานและคะแนน sits directly after บทเรียน, and คะแนน SGS is the last tab', () => {
    const keys = REAL_TABS.map((tab) => tab.key)
    expect(keys.indexOf('checkAndGrades')).toBe(keys.indexOf('lessons') + 1)
    expect(keys.indexOf('sgsScores')).toBe(keys.length - 1)
  })

  it('demo workspace still has its OWN standalone งาน/คะแนน tabs — demo mode is intentionally untouched by the real-mode งาน consolidation AND has no คะแนน SGS tab at all (SGS Bridge is a real-Supabase-data-only feature); every tab common to both still matches key-for-key and label-for-label', () => {
    ...
    expect(DEMO_TABS.some((tab) => (tab.key as string) === 'sgsScores')).toBe(false)
    const demoWithoutMergedAway = DEMO_TABS.filter((tab) => tab.key !== 'assignments' && tab.key !== 'grades')
    const realWithoutMergedTab = REAL_TABS.filter((tab) => tab.key !== 'checkAndGrades' && tab.key !== 'sgsScores')
